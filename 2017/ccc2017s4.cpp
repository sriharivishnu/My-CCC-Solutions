...
// Use the maximum weighted edge of the MST to recompute the MST, now
// considering the Enhancer Notice that the enhancer must zero one of the active
// edges not in the previous MST in order to at most decrease the day cost by 1.
// Used disjoint set with path compression and weighted union to optimize
// Kruskal's
...
bool find(int arr[], int x, int y) { return root(arr, x) == root(arr, y); }
int main() {
    int n, m, d;
    int x, y, w;
    int days = 0, c = 0;
    ...
    if (!maxEdge.original) {
        for (int e = 0; e < edges.size(); e++) {
            ...
            if (!find(connected, edge.x, edge.y)) {
                if (edge.w < maxEdge.w ||
                    ((edge.w == maxEdge.w) && edge.original)) {
